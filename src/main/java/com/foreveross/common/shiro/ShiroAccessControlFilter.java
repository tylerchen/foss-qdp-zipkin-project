...
public class ShiroAccessControlFilter extends AdviceFilter {
	...
	protected boolean preHandle(ServletRequest servletRequest, ServletResponse servletResponse) throws Exception {
		...
		{
			if (subject.getPrincipal() == null || !subject.isAuthenticated()) {
				String unauthorizedUrl = ConstantBean.getProperty("shiro.redirect.unauthorized.url", "");
				String toUrl = ConstantBean.getProperty("shiro.redirect.to.url", "");
				if (StringUtils.isNotEmpty(unauthorizedUrl) && StringUtils.isNotEmpty(toUrl)) {
					String[] unauthorizedUrls = StringUtils.split(unauthorizedUrl, ',');
					boolean match = false;
					...
					if (match) {
						Logger.debug(FCS.get("Unauthorized url configured and redirect: {0} to {1}.", url, toUrl));
						response.reset();
						...
						Logger.debug(
								FCS.get("Unauthorized url configured and not match to redirect: {0} not match {1}.",
										url, unauthorizedUrl));
					}
				}
				response.reset();
